...
{
    /// <summary> Wszystkie sale lub przefiltrowane wg parametrów zapytania. </summary>
    [HttpGet]
    ...
    public ActionResult<Room> PobierzPoId([FromRoute] int id)
    {
        ...
        if (sala is null)
            return NotFound($"Nie znaleziono sali o identyfikatorze {id}.");
        return Ok(sala);
    }
    ...
    public IActionResult Usun([FromRoute] int id)
    {
        ...
        if (MagazynDanych.Rezerwacje.Any(r => r.RoomId == id))
        {
            return Conflict(
                "Nie można usunąć sali, dla której istnieją rezerwacje. Usuń najpierw powiązane rezerwacje.");
        }
